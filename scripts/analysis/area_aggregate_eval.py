...
# Tier-1 statistical kernel was extracted to core.area_metrics (2026-06-12).
# Re-exported here so existing by-name callers
# (`from scripts.analysis.area_aggregate_eval import summarize`, etc.) keep
# resolving without code changes. evaluate_run / I/O stays in this module.
from core.area_metrics import (  # noqa: F401  (re-export for backward compat)
    _bootstrap_ci,
    _ols_regression,
    summarize,
)
    ...
    for region_key in regions:
        ...
        for run_id in runs:
            if args.run and run_id not in args.run:
                continue
            if args.skip_deprecated and get_model_run(region_key, run_id).deprecated:
