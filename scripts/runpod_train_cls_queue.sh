...
SOURCE_DATA_DIR="${SOURCE_DATA_DIR:-/root/cls_data/cls_pv_thermal_v1}"
TMPFS_DATA_DIR="${TMPFS_DATA_DIR:-/dev/shm/cls_data/cls_pv_thermal_v1}"
...
for arch in $RUNS; do
  case "$arch" in
    convnext_tiny)
      ...
      run_id="cls_pv_thermal_v1_convnext_tiny"
      ;;
    dinov2_vits14)
      config="configs/classifier/dinov2_vits14.json"
      run_id="cls_pv_thermal_v1_dinov2_vits14"
      ;;
    resnet18)
      config=""
      run_id="cls_pv_thermal_v1_resnet18"
      ;;
    *)
